...
class IntelligenceStore:
    ...
    def start_run(self, lead_id: int, source_url: str) -> str:
        ...
        try:
            conn.execute(
                """
                INSERT INTO analysis_runs (run_id, lead_id, source_url, status, started_at)
                VALUES (?, ?, ?, 'RUNNING', ?)
                """,
    ...
    def finish_run(self, run_id: str, status: str, summary: dict | None = None) -> None:
        ...
        try:
            conn.execute(
                ...
                WHERE run_id = ?
                """,
                (status, self._now(), self._json(summary), run_id),
            )
            conn.commit()
        finally:
            conn.close()
    ...
    def fail_run(self, run_id: str, error_type: str, error_message: str) -> None:
        ...
        try:
            conn.execute(
                ...
                WHERE run_id = ?
                """,
                (self._now(), error_type, error_message, run_id),
            )
            conn.commit()
        finally:
            conn.close()
        ...
        try:
            cursor = conn.execute(
                ...
                INSERT INTO audit_events
                    (run_id, lead_id, event_name, stage, severity, message, attributes_json, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
        ...
        try:
            cursor = conn.execute(
                ...
                INSERT INTO analysis_pages
                    (run_id, url, status, method, content_length, error_type, error_message, metadata_json, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
        ...
        try:
            cursor = conn.execute(
                ...
                INSERT INTO intelligence_observations
                    (run_id, page_url, kind, key, value_json, evidence_json, confidence, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
        ...
        try:
            cursor = conn.execute(
                ...
                INSERT INTO intelligence_signals
                    (run_id, name, classification, status, confidence, evidence_json, context_json, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
        ...
        try:
            cursor = conn.execute(
                ...
                INSERT INTO unknown_signals
                    (run_id, fingerprint, page_url, observation_json, context_json,
                     interpretation_json, validation_status, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
                    ...
                    self._json(context),
                    self._json(interpretation) if interpretation else None,
                    validation_status,
                    now,
                    now,
                ),
        ...
        try:
            cursor = conn.execute(
                ...
                INSERT INTO ai_interpretations
                    (run_id, purpose, provider, model, input_hash, status, confidence,
                ...
                (
                    ...
                    status,
                    confidence,
                    self._json(input_summary),
                    self._json(output) if output else None,
                    error_message,
                    self._now(),
                ),
            )
        ...
        try:
            cursor = conn.execute(
                ...
                INSERT INTO intelligence_opportunities
                    (run_id, opportunity_type, title, priority, score, confidence, impact,
                     solution_fit, evidence_json, unknowns_json, recommendation, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
                    ...
                    opportunity.get("impact"),
                    opportunity.get("solution_fit"),
                    self._json(opportunity.get("evidence", [])),
                    self._json(opportunity.get("unknowns", [])),
