    ...
    try:
        while True:
            ...
            try:
                ...
                audit.event(run_id, lead_id, "crawl.enrichment_completed", stage="crawl", website=final_website, company_name=enriched_lead.company_name)
                ...
                audit.event(run_id, lead_id, "crawl.completed", stage="crawl", pages=len(scraped_data.get("pages", [])), method=scraped_data.get("method", "unknown"), text_length=len(scraped_data.get("text", "")), emails=len(scraped_data.get("emails", [])), phones=len(scraped_data.get("phones", [])))

                db.update_lead_scraped_data(lead_id, emails=", ".join(scraped_data["emails"]), phones=", ".join(scraped_data["phones"]), text=scraped_data["text"])
                ...
                for page in scraped_data.get("page_details", []):
                    ...
                    compact_dom = {
                        "metadata": dom.get("metadata", {}), "headings": dom.get("headings", {}), "links": dom.get("links", []),
                        "buttons": dom.get("buttons", []), "forms": dom.get("forms", []), "json_ld": dom.get("json_ld", []),
                        "technologies": dom.get("technologies", []), "external_domains": dom.get("external_domains", []),
                        "attribute_samples": dom.get("attribute_samples", []),
                    }
                    intelligence.record_observation(run_id, page_url=page_url, kind="dom_profile", key="page_intelligence", value=compact_dom, evidence={"source": "HTMLProcessor", "page": page_url}, confidence=1.0)
                    audit.event(run_id, lead_id, "dom.observed", stage="extraction", page=page_url, links=len(dom.get("links", [])), forms=len(dom.get("forms", [])), buttons=len(dom.get("buttons", [])), technologies=len(dom.get("technologies", [])), external_domains=len(dom.get("external_domains", [])))

                analysis = analyzer.analyze(final_website, text=scraped_data.get("text", ""), scraped_data=scraped_data)
                audit.event(run_id, lead_id, "signals.analyzed", stage="signals", signal_count=sum(1 for value in analysis.get("signals", {}).values() if value))

                for signal_name, signal_value in analysis.get("signals", {}).items():
                    if signal_value:
                        intelligence.record_signal(run_id, name=signal_name, classification="KNOWN", confidence=1.0, evidence=[{"source": "business_analyzer", "value": signal_value}], context={"industry": analysis.get("industry", "unknown")})
                        audit.event(run_id, lead_id, "signal.created", stage="signals", signal=signal_name, classification="KNOWN")

                analysis["business_intelligence"] = analyzer.opportunity_detector.analyze_business(
                    signals=analysis["signals"], text=scraped_data.get("text", ""), pages=scraped_data.get("pages", []), industry=analysis["industry"], services=analysis.get("services", []),
                )
                ...
                for opportunity in analysis["opportunities"]:
                    ...
                    audit.event(run_id, lead_id, "opportunity.created", stage="opportunity", opportunity_type=opportunity.get("type"), title=opportunity.get("title"), score=opportunity.get("score"), confidence=opportunity.get("confidence"))
                ...
                if personalizer.router.available and analysis["opportunities"]:
                    ...
                    try:
                        ...
                        if personalizer.last_response:
                            ...
                            intelligence.record_ai(run_id, purpose="outreach_draft", provider=personalizer.last_response.provider, model=personalizer.last_response.model, input_hash=_hash_payload({"opportunities": analysis["opportunities"]}), status="COMPLETED", input_summary={"business_name": analysis.get("business_name"), "opportunity_count": len(analysis["opportunities"])}, output={"draft": draft})
                            audit.event(run_id, lead_id, "ai.personalization.completed", stage="ai", provider=personalizer.last_response.provider, model=personalizer.last_response.model)
                    except Exception as ai_exc:
                        analysis["personalized_email_error"] = str(ai_exc)
                        intelligence.record_ai(run_id, purpose="outreach_draft", provider=None, model=None, input_hash=_hash_payload({"opportunities": analysis["opportunities"]}), status="FAILED", input_summary={"business_name": analysis.get("business_name"), "opportunity_count": len(analysis["opportunities"])}, error_message=str(ai_exc))
                        audit.event(run_id, lead_id, "ai.personalization.failed", stage="ai", severity="WARNING", error_type=type(ai_exc).__name__)
                        logger.warning("AI personalization failed for Lead ID %s: %s", lead_id, ai_exc)

                db.update_lead_analysis(lead_id, opportunity_score=analysis["opportunity_score"], opportunity_data=json.dumps(analysis))
                ...
                audit.event(run_id, lead_id, "analysis.failed", stage="analysis", severity="ERROR", message=str(exc), error_type=type(exc).__name__)
